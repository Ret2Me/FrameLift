...
RUN_ROOT="$PROJECT_ROOT/work/prospective-v4a"
REPORT_ROOT="$PROJECT_ROOT/reports/observation-planning-prospective-v4a"
...
"$PYTHON" -m telemetry_yield.cli planning-prospective-reconcile-shadow \
  --config "$PROJECT_ROOT/configs/prospective/observation-planning-shadow-v4a.json" \
...
if (( NOW_EPOCH < CAMPAIGN_END_EPOCH )); then
  ...
  "$PYTHON" -m telemetry_yield.cli planning-prospective-run-shadow \
    --config "$PROJECT_ROOT/configs/prospective/observation-planning-shadow-v4a.json" \
    --publication-config "$PROJECT_ROOT/configs/observation-planning-publication-v3.json" \
...
if [[ -f "$HISTORICAL_REPORT_ROOT/manuscript/results-manifest.json" ]]; then
  "$PYTHON" -m telemetry_yield.cli planning-publication-readiness \
    ...
    --prospective-config "$PROJECT_ROOT/configs/prospective/observation-planning-shadow-v4a.json" \
