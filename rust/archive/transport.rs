//! Native campaign transport, promoted from the qualified acquisition transport v2: fixed HTTPS hosts, recorded public DNS, bounded retries and rate control.
use crate::input;
...
fn scoped_dns(host: &str, parent: &Path) -> Result<std::net::Ipv4Addr, String> {
    type DnsCache = BTreeMap<String, (Instant, std::net::Ipv4Addr, Value)>;
    ...
    if let Some((when, ip, _)) = cache.get(host)
        && when.elapsed() < Duration::from_secs(3600)
    {
        return Ok(*ip);
    }
