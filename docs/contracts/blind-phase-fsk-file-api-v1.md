# Blind phase FSK file API v1

## Boundary

`telemetry-yield-blind-phase-fsk-file-api-v1` is the stable batch boundary for
...
`schemas/blind-phase-fsk-file-result-v1.schema.json`.
...
closure; an exact completed attempt is returned without rerunning DSP. A
mismatched or corrupt existing result fails closed. This observation-level
idempotence is the v1 resume boundary; there is no within-observation
...
frontend pass. The file API rejects decoder windows above 5,000,000 complex samples, more than
...
result verification bounded. Rates with at least two source
...
retained per event, and at most 16 per window; event clustering is limited to
...
telemetry-yield decode-blind-phase-fsk /data/capture.raw \
  --api-version telemetry-yield-blind-phase-fsk-file-api-v1 \
...
The default v1 frontend bank uses radius factor 3, phase-difference lag 1, and
...
v1 does not silently search an unbounded range. The result schema freezes every
