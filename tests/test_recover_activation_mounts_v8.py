...
SOURCE = (
    ...
    / "work/blind-phase-confirmatory-v2/recover_activation_mounts_v8.py"
)
SPEC = importlib.util.spec_from_file_location("recover_activation_mounts_v8", SOURCE)
...
def test_v5_project_only_runner_can_invoke_only_project_umount(
    ...
    assert str(RECOVERY.V8_RESUME_PATH) in journal
...
def test_v5_missing_v3_candidate_journal_fails_before_command(
    ...
    assert set(journal) == {str(RECOVERY.V8_FAILURE_PATH)}
    assert calls == []
...
def test_v7_nonzero_binary_streams_survive_losslessly_in_durable_lockout(
    ...
    assert len(calls) == 1
    failure = journal[str(RECOVERY.V8_FAILURE_PATH)][0]
...
def _assert_v8_second_invocation_locked(
...
def test_v8_exit0_partial_fanout_preserves_receipt_and_locks_retry(
    monkeypatch, tmp_path: Path
) -> None:
    candidate = _state("candidate")
    ...
    assert len(calls) == 1
    failure = journal[str(RECOVERY.V8_FAILURE_PATH)][0]["command_failure"]
    ...
    _assert_v8_second_invocation_locked(monkeypatch, tmp_path, journal, partial)


def test_v8_exit0_post_capture_exception_preserves_receipt_and_locks_retry(
    monkeypatch, tmp_path: Path
) -> None:
    candidate = _state("candidate")
    journal, calls, runner = _install_run_mocks(
    ...
    _assert_v8_second_invocation_locked(monkeypatch, tmp_path, journal, candidate)


def test_v8_exit0_target_identity_drift_preserves_receipt_and_locks_retry(
    monkeypatch, tmp_path: Path
) -> None:
    candidate, final = _state("candidate"), _state("final")
    journal, calls, runner = _install_run_mocks(
    ...
    _assert_v8_second_invocation_locked(monkeypatch, tmp_path, journal, final)


def test_v8_exit0_completion_publication_error_preserves_receipt_and_locks_retry(
    monkeypatch, tmp_path: Path
) -> None:
    candidate, final = _state("candidate"), _state("final")
    journal, calls, runner = _install_run_mocks(
...
def test_v7_failure_marker_publication_collision_still_locks_out(
    ...
    def collide(path, document):
        if path == RECOVERY.V8_FAILURE_PATH:
    ...
    assert calls == []
    assert str(RECOVERY.V8_FAILURE_PATH) in journal
