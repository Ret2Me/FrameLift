...
SOURCE = (
    ...
    / "work/blind-phase-confirmatory-v2/recover_activation_mounts_v5.py"
)
SPEC = importlib.util.spec_from_file_location("recover_activation_mounts_v5", SOURCE)
...
def test_v5_project_only_runner_can_invoke_only_project_umount(
    ...
    assert str(RECOVERY.V5_RESUME_PATH) in journal
...
def test_v5_existing_ready_and_candidate_runs_exactly_one_project_call(
    monkeypatch, tmp_path: Path
) -> None:
    candidate, final = _state("candidate"), _state("final")
    journal, calls, runner = _install_run_mocks(
    ...
    assert result["transition"] == "executed"
    assert [call[0] for call in calls] == [
        [str(RECOVERY.UMOUNT), "--no-canonicalize", RECOVERY.PROJECT]
    ]
