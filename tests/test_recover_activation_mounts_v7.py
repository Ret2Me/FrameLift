...
SOURCE = (
    ...
    / "work/blind-phase-confirmatory-v2/recover_activation_mounts_v7.py"
)
SPEC = importlib.util.spec_from_file_location("recover_activation_mounts_v7", SOURCE)
...
def test_v5_project_only_runner_can_invoke_only_project_umount(
    ...
    assert str(RECOVERY.V7_RESUME_PATH) in journal
...
def test_v5_missing_v3_candidate_journal_fails_before_command(
    ...
    assert set(journal) == {str(RECOVERY.V7_FAILURE_PATH)}
    assert calls == []
...
def test_v7_nonzero_binary_streams_survive_losslessly_in_durable_lockout(
    ...
    assert len(calls) == 1
    failure = journal[str(RECOVERY.V7_FAILURE_PATH)][0]
...
def test_v7_failure_marker_publication_collision_still_locks_out(
    ...
    def collide(path, document):
        if path == RECOVERY.V7_FAILURE_PATH:
    ...
    assert calls == []
    assert str(RECOVERY.V7_FAILURE_PATH) in journal
