...
fn plan() -> Value {
    let mut value = previous_plan();
    value["schema"] = "innovation-benchmark-selection-v3".into();
...
fn replay(out: &Path, original: &Path) -> Result<Value, String> {
    ...
    if old["schema"] != "innovation-benchmark-cohort-v2" || old["plan"] != previous_plan() {
    ...
    let amendment = json!({"schema":"station-field-correction-v3","created_utc":io::utc(),
        ...
        "original_prior_exposure":old["prior_exposure"],"new_station_field":"ground_station",
    ...
    input::write_json_new(&out.join("cohort.json"), &cohort)?;
    let label = original.join("original-selection-not-used-v3.json");
    if !label.exists() {
        input::write_json_new(
            &label,
...
fn run(args: Args) -> Result<(), String> {
    let cohort = if args.resume {
        ...
        let original = args
            ...
            .as_deref()
            .ok_or("v3 only replays full frozen metadata; no direct API acquisition allowed")?;
...
mod tests {
    ...
    fn full_snapshot_replay_is_new_and_partial_prefix_cannot_freeze() {
        for eof in [true, false] {
            ...
            input::write_json_new(&original.join("metadata-all.json"),&json!({"observations":rows,"pages":[{"tier":0,"next":if eof {Value::Null}else{json!("nextpage")}}]})).unwrap();
            ...
            .unwrap();
            let old = json!({"schema":"innovation-benchmark-cohort-v2","plan":previous_plan(),"metadata_page_count":1,
            ...
            if eof {
                ...
                assert!(
                    original
                        .join("original-selection-not-used-v3.json")
                        .exists()
                );
