//! Corrected v6: metadata-only continuation with exact request/final/hop URL checks.
...
fn verified_page(
    ...
    selector::exact_metadata_http(&http, &header_text, expected_url)?;
...
mod tests {
    ...
    fn changed_or_missing_page_fails_closed() {
        ...
        let receipt = json!({"success":true,"requested_url":url,"result":{"identity":input::identity(&body).unwrap(),"url":url,"final_url":url},
            "hops":[{"url":url,"http_status":"200","hop":0,"attempt":0,"process":{"success":true}}]});
