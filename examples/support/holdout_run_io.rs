//! Bounded acquisition and process receipts for the frozen holdout runner.
...
pub fn disk_guard(path: &Path) -> Result<(), String> {
    ...
    if available < 21 * 1024 * 1024 * 1024 {
        return Err("disk guard: less than 20 GiB reserve plus 1 GiB worker allowance".into());
    }
    Ok(())
}
