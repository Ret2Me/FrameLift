...
RUN_ROOT="$PROJECT_ROOT/work/prospective-v4f"
REPORT_ROOT="$PROJECT_ROOT/reports/observation-planning-prospective-v4f"
...
"$PYTHON" -m telemetry_yield.cli planning-prospective-reconcile-shadow \
  --config "$PROJECT_ROOT/configs/prospective/observation-planning-shadow-v4f.json" \
...
if [[ -f "$HISTORICAL_REPORT_ROOT/manuscript/results-manifest.json" ]]; then
  "$PYTHON" -m telemetry_yield.cli planning-publication-readiness \
    ...
    --prospective-config "$PROJECT_ROOT/configs/prospective/observation-planning-shadow-v4f.json" \
