#!/usr/bin/env python3
"""Small development ablations: exact fast AX25 and additive clock diversity."""
...
from telemetry_yield.generic_receiver import Ax25ProtocolDecoder, GenericReceiver, ReceiverHypothesis, WaveformHypothesis
...
def main() -> int:
    ...
    parser.add_argument("--decoder", choices=("reference", "fast"), default="fast")
    parser.add_argument("--bank", choices=("global", "diverse", "full"), default="diverse")
    args = parser.parse_args()
    receiver = DiverseTimingReceiver() if args.bank == "diverse" else GenericReceiver()
    if args.decoder == "fast":
        from telemetry_yield.fast_ax25 import FastAx25ProtocolDecoder
        protocol = FastAx25ProtocolDecoder
    else:
        protocol = Ax25ProtocolDecoder
    receiver.register_protocol(protocol(protocol_id="ax25", g3ruh_modes=(False, True)))
    receiver.register_protocol(protocol(protocol_id="ax25_plain", g3ruh_modes=(False,)))
    ...
    plan = (ReceiverHypothesis(WaveformHypothesis(
        ...
        phase_bins=32, top_timing_hypotheses=160 if args.bank == "full" else args.top_timing,
        modulation_family=args.mode), "ax25" if is_fsk else "ax25_plain"),)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    config = {"schema": "native-audio-refinement-plan-v2", "decoder": args.decoder, "clock_bank_policy": args.bank, "development_not_holdout": True, "input": file_identity(args.input),
              ...
              "source_dependencies": [file_identity(Path(__file__).resolve().parents[1] / "src/telemetry_yield" / name)
                  for name in ("fast_ax25.py", "diverse_timing.py", "generic_receiver.py", "clock_recovery.py",
                               "ax25_validation.py", "crc.py", "camras_replay.py", "afsk1200_plugin.py")]}
    ...
    with sf.SoundFile(args.input) as audio, journal.open("x") as log:
        for offset in range(0, info.frames, hop):
            ...
            result = decode_audio_pcm(samples, sample_rate_hz=sample_rate,
                                      representation="fm_demodulated", plan=plan, receiver=receiver)
    ...
    summary = {"schema": "native-audio-refinement-result-v2", "decoder": args.decoder, "clock_bank_policy": args.bank, "input": config["input"],
