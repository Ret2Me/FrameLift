...
SOURCE = (
    ...
    / "work/blind-phase-confirmatory-v2/recover_activation_mounts_v10.py"
)
SPEC = importlib.util.spec_from_file_location("recover_activation_mounts_v10", SOURCE)
...
def test_v5_project_only_runner_can_invoke_only_project_umount(
    ...
    assert str(RECOVERY.V10_RESUME_PATH) in journal
...
def test_v5_missing_v3_candidate_journal_fails_before_command(
    ...
    assert set(journal) == {str(RECOVERY.V10_FAILURE_PATH)}
    assert calls == []
...
def test_v7_nonzero_binary_streams_survive_losslessly_in_durable_lockout(
    ...
    assert len(calls) == 1
    failure = journal[str(RECOVERY.V10_FAILURE_PATH)][0]
...
def _assert_v10_second_invocation_locked(
...
def test_v10_exit0_partial_fanout_preserves_receipt_and_locks_retry(
    monkeypatch, tmp_path: Path
) -> None:
    candidate = _state("candidate")
    ...
    assert len(calls) == 1
    failure = journal[str(RECOVERY.V10_FAILURE_PATH)][0]["command_failure"]
    ...
    _assert_v10_second_invocation_locked(monkeypatch, tmp_path, journal, partial)


def test_v10_exit0_post_capture_exception_preserves_receipt_and_locks_retry(
    monkeypatch, tmp_path: Path
) -> None:
    candidate = _state("candidate")
    journal, calls, runner = _install_run_mocks(
    ...
    _assert_v10_second_invocation_locked(monkeypatch, tmp_path, journal, candidate)


def test_v10_exit0_target_identity_drift_preserves_receipt_and_locks_retry(
    monkeypatch, tmp_path: Path
) -> None:
    candidate, final = _state("candidate"), _state("final")
    journal, calls, runner = _install_run_mocks(
    ...
    _assert_v10_second_invocation_locked(monkeypatch, tmp_path, journal, final)


def test_v10_exit0_completion_publication_error_preserves_receipt_and_locks_retry(
    monkeypatch, tmp_path: Path
) -> None:
    candidate, final = _state("candidate"), _state("final")
    journal, calls, runner = _install_run_mocks(
...
def test_v10_exit0_complete_then_pin_close_failure_retains_receipt_and_completion(
    monkeypatch, tmp_path: Path
) -> None:
    candidate, final = _state("candidate"), _state("final")
...
def test_v10_postcapture_primary_plus_pin_close_failure_preserves_both_errors(
    monkeypatch, tmp_path: Path
) -> None:
    candidate = _state("candidate")
    journal, calls, runner = _install_run_mocks(
...
def test_v10_precommand_primary_plus_pin_close_failure_preserves_both_without_receipt(
    monkeypatch, tmp_path: Path
) -> None:
    candidate = _state("candidate")
    journal, calls, runner = _install_run_mocks(
    ...
    assert failure["cleanup_failure"] == {
        "primary_error": {
            "type": "RecoveryError",
            "message": (
                "v10 is project-only and requires exact durable v3 intent "
...
def test_v10_normal_success_closes_pins_once_without_failure_lockout(
    monkeypatch, tmp_path: Path
) -> None:
    candidate, final = _state("candidate"), _state("final")
    journal, calls, runner = _install_run_mocks(
    ...
    assert len(calls) == 1
    assert len(closes) == 1
    assert str(RECOVERY.V10_FAILURE_PATH) not in journal
...
def test_v7_failure_marker_publication_collision_still_locks_out(
    ...
    def collide(path, document):
        if path == RECOVERY.V10_FAILURE_PATH:
    ...
    assert calls == []
    assert str(RECOVERY.V10_FAILURE_PATH) in journal
...
def test_open_namespace_pin_fstat_and_close_failure_preserve_both_once(
    ...
    assert type(raised.value.cleanup) is RECOVERY.RecoveryError
